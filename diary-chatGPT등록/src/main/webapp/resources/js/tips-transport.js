						...
						$('#tips-content-more').append(
								`<ul class="tips-contents">
									...
									<span style="display:none" class="tips-user-idx">${item.user_idx}</span>
								  	...
								  	<span style="display:none;">${item.transport_idx}</span>
								  </ul>
								 `
						)
...
$(document).on('click','.tips-btn',function(){
		...
		var tips_transport = children.eq(1).text();
...
function Comment(){
	$(document).on('click','.tips-comment',function(){
			...
			var children = parent.children();
			var transport_idx = children.eq(5).text();
			
			$('#tips-content-more').empty();
			$('#pagination-transport').empty();
			
			$.ajax({
				url:'./tips/getTransForCom',
				type:'get',
				async:false,
				...
				success:function(trans){
					$('#tips-content-more').append(
							`<ul class="tips-contents">
								...
								<span class="tips-contents-more">${trans.tips_transport}</span>
								<span class="tips-comment-back"><i class="fa-solid fa-rotate-left">돌아가기</i></span>
							  </ul>
							  <div class="tips-comments">
							  	<div class="comment-write">
							  		<span style="display:none;">${trans.transport_idx}</span>
					...
					$.each(list,function(index,item){
						...
						if(item.nickname==nickname){
							$('.tips-comments').append(
									`<div class="comments">
											<span class="comment-idx" style="display:none">${item.transport_comment_idx}</span>
									  		<span class="comments-user">${item.nickname}</span>
									  		<span class="comments-desc">${item.transport_comment}</span>
...
function addComment(){
	$(document).on('click','.upload-comment',function(){
			...
			var parent = $(this).parent();
			var children = parent.children();
			var transport_idx = children.eq(0).text();
	
			var transport_comment = $('.comment').val();
