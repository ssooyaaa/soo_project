						...
						$('#photo-section-row').append(
								`<div class="col-4">
									<div class="photo-card">
										...
										<span style="display:none" class="abroad-idx">${item.abroad_idx}</span>
...
function addPhreport(){
	...
	$(document).on('click','.photos-btn',function(){
		...
		var domestic_sight_img = '';
		var domestic_sight_desc = '';
		var abroad_sight_img = children.eq(0).attr('src');
		var abroad_sight_desc = children.eq(2).text();
...
function Comment(){
	$(document).on('click','.photos-comment',function(){
			...
			var children = parent.children();
			var abroad_idx = children.eq(4).text();
			
			$('#photo-section-row').empty();
			$('#pagination-abroad').empty();
			
			$.ajax({
				url:'./photos/getAbForCom',
				type:'get',
				async:false,
					...
					$('#photo-section-row').append(
							`<div class="col-4">
								<div class="photo-card">
									...
									<span class="photos-nation">[${ab.nation}]</span>
									<p>${ab.sight_desc}</p>
									<span style="display:none" class="photos-user-idx">${ab.user_idx}</span>
									<span style="display:none" class="domestic-idx">${ab.abroad_idx}</span>
									<span class="photos-comment"><i class="fa-regular fa-comment-dots"> 댓글 작성</i></span>
									<span class="photos-btn">신고</span>
								</div>
							</div>
							<div class="col-8 photos-comments">
								<span class="photos-comment-back"><i class="fa-solid fa-rotate-left">돌아가기</i></span>
							  	<div class="comment-write">
							  		<span style="display:none;">${ab.abroad_idx}</span>
					...
					$.each(list,function(index,item){
						...
						if(item.nickname==nickname){
							$('.photos-comments').append(
									`<div class="comments">
											<span class="comment-idx" style="display:none">${item.abroad_comment_idx}</span>
									  		<span class="comments-user">${item.nickname}</span>
									  		<span class="comments-desc">${item.abroad_comment}</span>
...
function addComment(){
	$(document).on('click','.upload-comment',function(){
			...
			var parent = $(this).parent();
			var children = parent.children();
			var abroad_idx = children.eq(0).text();
	
			var abroad_comment = $('.comment').val();
